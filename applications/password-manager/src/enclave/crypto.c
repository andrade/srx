...
#include "crypto.h"
#include "utils.h"
#include "sqlite3.h"  // for sqlite3_malloc64(); ideal é caller alocar... TODO
...
uint8_t titan_load_data(const char *path, const uint8_t *key, uint8_t **data, size_t *size)
{
	...
	if (sz < 1 || sz > 1024 * 1024) {
		fprintf(stderr, "broke size boundaries\n");
		goto evil_release;
	}
	printf("read size is = %"PRId64"\n", sz);

	uint8_t *p = sqlite3_malloc64(sz); // deserialize com auto free on close
	// uint8_t *p = malloc(sz);
	if (!p)
		goto evil_release;
...
//@deprecated   No longer using Protected FS Library
uint8_t titan_save_data(const char *path, const uint8_t *key, const uint8_t *data, size_t size)
{
