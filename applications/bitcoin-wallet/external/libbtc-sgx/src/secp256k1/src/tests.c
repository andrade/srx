...
#if ENABLE_SGX
    int b = sgx_is_within_enclave(seed16, sizeof(seed16));/* FIXME to rm */
    printf("sgx_is_within_enclave(seed16)=%d\n", b);
#endif
...
#else /* ENABLE_SGX */
    if (argc > 2) {
        fprintf(stderr, "sscanf not implemented in SGX\n");
        abort();
#if 0
        /* TODO to be implemented, sscanf N/A */
        int pos = 0;
        const char* ch = argv[2];
        while (pos < 16 && ch[0] != 0 && ch[1] != 0) {
            unsigned short sh;
        ...
        ss = sgx_read_rand(seed16, sizeof(seed16));
        if (ss != SGX_SUCCESS)
            abort();
    }
#endif /* ENABLE_SGX */
